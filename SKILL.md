...
The `references/` files are generated by the [synchronizer](scripts/sync.mjs). Do not hand-edit them. Exact upstream revisions are emitted by the synchronizer and recorded by automated refresh commits. A workflow that upstream has removed or replaced with a thinner procedure is frozen at the last upstream revision that published it, and its generated file says so at the top. Stable source and output hashes, plus every freeze, live in the [source manifest](references/source-manifest.json). Authored maintenance rules live in `AGENTS.md`.
